    ...
    try:
        ...
        for ticker in tickers:
            if start_time < now < end_time - datetime.timedelta(seconds=30):
                if get_balance(ticker)>0:
                    continue
                elif:
                    target_price = get_target_price(money+ticker, 0.5)
                    current_price = get_current_price("KRW"+ticker)
                    if target_price < current_price and current_price < predicted_close_price:
                        krw = get_balance("KRW")
                        if krw > 5000:
                            upbit.buy_market_order("KRW-"+ticker, 500000*0.9995)
            else:
                coin = get_balance(ticker)
                if coin > 0.00008:
                    upbit.sell_market_order("KRW-"+ticker, coin*0.9995)
            time.sleep(1)
